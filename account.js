...
async function loadRequests() {
  ...
  $('accountRequests').innerHTML = rows.map((row) => `
    <div class="account-item request-item">
      ...
      <div class="request-actions">
        ${row.professional_id ? `<button class="request-action accept" type="button" data-chat-request="${row.id}">Abrir conversa</button>` : ''}
  ...
  document.querySelectorAll('[data-chat-request]').forEach((button) => {
    button.onclick = () => { closeAccount(); openServiceChat(button.dataset.chatRequest).catch(console.error); };
  });
...
async function loadProfessional() {
  ...
  const [profileResult, linksResult, requestsResult] = await Promise.all([
    A.from('professional_profiles').select('public_name,headline,city,is_public,is_available').eq('user_id', session.user.id).maybeSingle(),
    ...
    <div class="account-list">
      ${active.length ? active.map((row) => `
        <div class="account-item received-request">
          ...
          <small>${escapeHtml(row.description)}${row.city ? ` · ${escapeHtml(row.city)}` : ''}</small>
          <button class="request-action accept" type="button" data-chat-request="${row.id}">Abrir conversa</button>
        </div>
      `).join('') : '<div class="account-empty">Não tens pedidos ativos neste momento.</div>'}
    </div>
    <h3>Serviços concluídos</h3>
    <div class="account-empty">${completed.length ? `${completed.length} serviço(s) concluído(s).` : 'Ainda não tens serviços concluídos.'}</div>
  `;
...
async function refreshProfessionalBadge(notify = true) {
  if (!session) { applyProfessionalBadge(0); return; }
  const { count, error } = await A.from('service_requests')
    .select('id', { count: 'exact', head: true })
    .eq('professional_id', session.user.id)
    .in('status', ['open', 'matched']);
  if (error) return;
  const next = count || 0;
  if (notify && lastIncomingCount !== null && next > lastIncomingCount) showToast('Novo pedido recebido');
  lastIncomingCount = next;
  applyProfessionalBadge(next);
}

function startNotifications() {
  if (notificationTimer) clearInterval(notificationTimer);
  refreshProfessionalBadge(false).catch(console.error);
  notificationTimer = setInterval(() => refreshProfessionalBadge(true).catch(console.error), 15000);
}
